...
Both lanes also keep a `givens.md` — the a-priori details the user brought, each `open`/`consumed`/`superseded`/`set-aside`, `decided` or `leaning`, routed to a node — and their logs carry a shared move, `given`: a turn volunteering a detail about a non-active node, filed to the ledger with the map otherwise untouched.
...
- `givens.md` — the user's a-priori details: status, conviction (`decided` held as the user's call once confirmed / `leaning` a default recommendation), and the node each routes to.
...
- **Givens** — the unconsumed, unrouted, or silently overridden givens, each with its remedy: the node to reopen, or the conflict to put to the user.
