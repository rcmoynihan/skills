...
**Review.** Dispatch `powerstorm-slice-reviewer` with the same context and the drafted slice. It returns focused review notes — gaps, contradictions with approved slices, missing acceptance criteria, vague contracts, anything that would block the next slice. If the notes are substantive, have the drafter revise (or revise directly with Edit), then **re-dispatch the reviewer on the revised slice** and revise again against that second pass. Run this review-and-revise cycle **twice**: draft → review → revise → review → revise. The independent check sees the revised slice, not only the pre-fix draft. If the review flags the slice's **core approach** as unsound or hand-wavy — not a local fix — dispatch `powerstorm-slice-thinker` to solve it, then re-draft against the new design note within these two rounds, rather than looping the drafter on a problem it is not equipped to crack. *Be patient* - do not cheat a review because an agent is taking a while.

Alongside "revise" and "escalate to the thinker", handle a third kind of finding: a **researchable-fact gap** — a fact the review marked unresolved that a public source could answer. Do not let such a fact reach the user as an open gap until one bounded research attempt has happened: have the drafter (which is web-enabled) look it up and fold the answer into the slice tagged `(verified: <url>)`. Only a fact the review tagged `Legitimately deferred` or `Needs user decision` — one that needs private/runtime access or a decision the user has not made — may surface to the user as still open. This runs within the two review rounds; it does not add rounds.

Return the finished slice for user review: report the target document path, and surface the slice's **Layer Story** prominently — that is the user's primary review aid. State plainly whether the slice is a **clean pass** or carries a **blocking finding** the caller must gate on — an upstream contradiction (the slice shows the direction, the brief, or the map was wrong), an invariant violation you could not resolve within your two rounds, or a fact you tagged "Needs user decision" — so the orchestrator knows whether it must stop. Keep every artifact a clean current-state description; never narrate the change, the review rounds, or your process across revisions.
