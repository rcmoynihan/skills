...
- **Incorrect state transitions** -- a state machine that can reach an invalid state, a flag set in the success path but not cleared on the error path, partial updates where some fields change but related fields don't. After-error state that leaves the system in a half-updated condition. Or a helper that mutates a caller-owned dict/object in place while that same object is also returned to the caller -- best-effort side work then silently alters the response; copy the object before mutating a shared reference.
