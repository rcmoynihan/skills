...
- **Tests that don't assert behavior (false confidence)** -- tests that call a function but only assert it doesn't throw, assert truthiness instead of specific values, mock so heavily that the test verifies the mocks, not the code, or feed input byte-identical to a fixture/gold record so the test passes by retrieving its own expected answer. The tell: would the test still pass if the code under test were reverted? If yes, it proves nothing. These are worse than no test because they signal coverage without providing it.
...
- **Behavior tested only below the shipped surface** -- a new endpoint or service-level path exercised only through helper/unit tests with the router or runtime patched out, so wiring, declared response models, and the validation envelope are never driven through the real stack. Confirm a test invokes the shipped path end-to-end, matching the coverage its sibling routes already have.
