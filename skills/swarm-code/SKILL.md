...
- **All agents are `model: inherit`.** Capability is expressed through `effort` tiering, not a hardcoded model hierarchy — the invoking user already chose their session model. A per-role `model:` pin is an optional user knob, never baked in.
