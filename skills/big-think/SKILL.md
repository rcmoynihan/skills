...
- **Dispatch agents by name.** The plugin ships five workers: `big-think-scout` (grounds one question across all surfaces + runs the gate's read-only probe), `big-think-diagnostician` (one diagnostic lens, dispatched once per lens), `big-think-red-team` (attacks the leading root-cause hypothesis at the gate), `big-think-idea` (generates one remedy, dispatched once per stance), and `big-think-premortem` (attacks the chosen remedy). Dispatch them with the Agent tool (`subagent_type`); if `/agents` shows them plugin-scoped, use that form. Workers never spawn Claude subagents — you own all fan-out. (A worker may consult an external model such as Codex as a read-only cross-check; that is a tool call, not a spawned agent.)
...
- **Cap the loops with hard limits.** Diagnosis: at most **2 fan-out rounds**. Gate: at most **2 red-team passes** and **3 scout probes** per leading hypothesis. Loop-backs (reopening the frame or the gate): at most **2** for the whole run. Convergence: **1 scoring round** plus the pre-mortem. When a cap is reached without convergence, stop and surface the impasse — an investigation plan, or a decision under acknowledged uncertainty — rather than looping again.
...
A **Complex** problem (Cynefin) replaces analysis-to-a-cause with **safe-to-fail probes**. Instead of the lens fan-out, dispatch the Scout to run **at most three or four** small, read-only, reversible experiments — each perturbing or observing one suspected factor — and record what each reveals in `hypotheses.md`. Probe findings become a gated hypothesis the normal way *only if* one factor's signal is strong enough to state a mechanism and a falsifiable prediction; if the signal stays diffuse, the honest output is an **experiment plan** in `investigation-plan.md` (the next probes to run), never a forced cause.

## Phase 0 — Triage & frame

**If big-think was invoked with no problem stated, ask the user for the symptom first — never invent one.** Otherwise create the run directory and write `problem.md` from the template below, filling it by provenance: what the user stated, what you verified from a source (tagged), what you are assuming (flagged, never baked into a field), what is still open. Establish scope and persona explicitly — do not silently import them. (Exception: if a quick first read shows this is a **Clear**, already-solved problem, skip the scaffolding entirely — give the known answer or runbook and stop, per below.)

Then do three things:

1. **Classify with Cynefin** and record it — it dials the whole run.
   - **Clear** (known, ordered): a solved kind of problem. Give the known answer or runbook directly and stop — don't create a run directory or spin up the machine.
   ...
   - **Chaotic** (crisis, no stable cause yet): don't diagnose first. Recommend the single stabilizing / containment action that calms the system, write it into `decision-record.md` as an immediate action, and note that once stable the problem is re-triaged (it usually lands in Complicated or Complex). Containment precedes cause.
...
Merge the returned hypotheses into `hypotheses.md` as first-class, scored, cross-linked entries. **Dedup by *mechanism*, not by wording** — collapse two lenses describing the same causal chain into one (noting the cross-lens agreement, which raises confidence), but keep genuinely distinct rivals separate even when they predict the same symptom. Where one hypothesis is a *symptom of* another, record the parent/child chain explicitly rather than merging them. This is the "Discover" diverge; you converge it at the gate.
...
On the **Complex path**, the "cause" is the pattern the probes revealed, and its falsifiable test is a further probe whose outcome it predicts; if no pattern is strong enough to meet every criterion, exit to `investigation-plan.md` as an experiment plan rather than weakening the criteria to force a pass.

**STOP (required).** Present `root-cause.md` (or `investigation-plan.md`). A plain **confirm** — or a wording/scope tweak that leaves the cause intact — advances to Phase 3. A **correction that changes the cause**, or a replacement the user proposes, is *not yet gated*: re-enter the gate for it (re-probe and re-red-team against the exit criteria) before any remedy work — a user's say-so is not a substitute for surviving the gate. Nothing proceeds to remedies without a *gated* cause. If the run ended in an investigation plan, this is where it ends.
