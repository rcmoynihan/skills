...
Posture is a **dial on depth, not on coverage.** It sets how hard you press each item and when an item is *resolved enough* — a `poc` item closes on a lighter answer, a `new-system` item isn't done until it's been pressed on the edge cases and promises the tier demands. It never decides whether a concern *exists*: the Planner may **add** the areas a heavier tier demands but never drops one because the tier is lean, which keeps the frozen initial agenda an honest coverage baseline. Breadth comes from the task itself: the Planner sizes the agenda to the delta being decided — its scale read at the top of `initial-agenda.md` — so a small fix walks a small map and a new system a full one. The Facilitator watches for posture mismatch both ways — over-grilling a PoC, or shallow-closing a serious system.

## Granularity — the run's decision-granularity mode

Posture dials how hard each decision is pressed; **granularity** dials how many of them the user personally adjudicates. It is one field in the Position block — `granularity: <autonomous | standard | paranoid>` — proposed in your opening move alongside the posture confirmation (not an extra turn) and resettable by the user any time with one word (logged as a `steer` row). Default `standard`; suggest `autonomous` when the invocation asks for speed or the posture sits at `poc`; suggest `paranoid` for `new-system` runs or when the user asks for maximum scrutiny.
...
- `autonomous` — you may **self-close** an item, but only when all three hold: **grounded** — Scout-verified, read directly from code/terrain/docs, or directly entailed by a given or the brief; **not load-bearing** — it wouldn't become an invariant and doesn't constrain other nodes; **not preference-shaped** — no product-taste, appetite, or priority call that belongs to the user. A self-close takes the normal depth tag plus a marker — `[resolved: mechanism (auto)]` — and its own log row, `move: auto`. **Digest-and-veto:** with the next question you ask, list every auto-close since the last digest ("Self-closed: B2 → …, D1 → … — veto any"); a veto reopens the node as a `correct`. **The hard floor — always asked, in any mode:** the posture and the scope boundary; anything that would become an invariant or non-negotiable; `decided`-given confirmations; and the gate adjudications.
...
- Position block: `posture` (the run's rigor tier — see above; carried here so every turn and the Facilitator read it for near-zero cost), `granularity` (the decision-granularity mode — see above), `terrain` (the terrain pack path when the run has one, else `—`), `active`, `resume-target` (the node to climb back to when the current tangent ends), `turns-since-node-change` (a counter you bump each turn and reset to 0 when the active node changes), `last-facilitator`.
...
4. **Update the map** — flip the active item's status if it resolved (**at most one standing item per user turn** — closing several on one answer is how a shallow answer slips through unexamined; if you're tempted to bulk-close, you're describing, not resolving; `auto` closes are exempt from the cap, but each carries its own log row and must appear in a digest before the gate), and flip its given to `[consumed]` when the resolution folded one in; add any new concern; rewrite the Position block (bump or reset `turns-since-node-change`, update `active` / `resume-target`).
...
- **Cadence:** `≥ 4` turns since `last-facilitator` (count from the start if it never ran) — tighten to `≥ 3` if any turn since then was a `tangent`, `new-concern`, or `correct`, or whenever `granularity: paranoid`. This is the one that fires when the active node changes *every* turn, which same-node staleness never catches.
...
- [resolved: mechanism (auto)] B3 <question> — <the behavior it resolved to; scout-verified>
- [paused: needs-review] B4 <question> — reason added: <standing rationale>
...
| 9 | D1 | auto | self-closed: export format matches domain norm | scout-verified; digest owed next question | none | G1 |
| 10 | F6 | meta | tracer: viewer journey collides F1×K5 | reopen F1, K5; who sees drafts is undecided | deep | F1 |
| 12 | F1 | steer | user: jump to E — value first | F1 paused; resume-target F1 | none | E1 |
```
