...
Seed the living agenda from it: copy the concern areas and items into `living-agenda.md` (given annotations ride along), every item `[unvisited]`, and add a fresh Position block (`posture:` set to the Planner's proposal, `terrain:` the pack path or `—`, `active: —`, `resume-target: —`, `turns-since-node-change: 0`, `last-facilitator: —`). The living copy must read as clean current state — no "copied from…" note.
