...
description: Delegate coding tasks to the Claude Code CLI (Anthropic's autonomous coding agent) from the shell — one-shot print-mode runs or multi-turn interactive tmux sessions for building features, refactors, and PR reviews. Use when the user asks to drive, orchestrate, or delegate work to Claude Code as a sub-agent.
disable-model-invocation: true
---

# Claude Code — Orchestration Guide

Delegate coding tasks to [Claude Code](https://code.claude.com/docs/en/cli-reference) (Anthropic's autonomous coding agent CLI) by running its CLI through the Bash tool. Claude Code v2.x can read files, write code, run shell commands, spawn subagents, and manage git workflows autonomously.

All commands below are run with the **Bash** tool. To orchestrate:

- **Run a command** — pass it to the Bash tool; set a generous `timeout` for long print-mode runs, and prefix `cd <dir>` to set the working directory.

- **Background a long task** — launch it with the Bash tool's `run_in_background: true`, read its output with the **BashOutput** tool, and stop it with **KillShell**.

- **Interactive (multi-turn) sessions** — the Bash tool has no PTY, so drive the TUI through tmux with `send-keys` (input) and `capture-pane` (monitoring), as shown below.


## Two Orchestration Modes

You can drive Claude Code in two fundamentally different ways. Choose based on the task.
...
- **Interactive mode REQUIRES tmux** — Claude Code is a full TUI app, and the Bash tool allocates no PTY. Run it inside tmux so you get `capture-pane` for monitoring and `send-keys` for input, which is essential for orchestration.
...
- **Always set the working directory** — prefix `cd <dir>` so Claude stays focused on the right project directory
