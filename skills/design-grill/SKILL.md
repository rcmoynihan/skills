...
- **Position block** — rewritten wholesale each turn: `posture` (the inherited tier), `terrain` (the terrain pack path when the run has one, else `—`), `active`, `resume-target`, `turns-since-node-change`, `last-facilitator`.
