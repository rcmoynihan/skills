...
## Granularity — the run's decision-granularity mode

Posture dials how hard each decision is pressed; **granularity** dials how many of them the user personally adjudicates. It is one field in the Position block — `granularity: <autonomous | standard | paranoid>` — a session preference, not a spec property: propose it in your opening move (not an extra turn), and the user resets it any time with one word (logged as a `steer` row). Default `standard`; suggest `autonomous` when the invocation asks for speed or the posture sits at `poc`; suggest `paranoid` for `new-system` runs or when the user asks for maximum scrutiny.

- `standard` — the loop exactly as written: one question at a time, your recommended answer with each, the user adjudicates every close.
- `autonomous` — you may **self-close** an item, but only when all three hold: **grounded** — Scout-verified, read directly from code/terrain/docs, or directly entailed by a given, the spec, or a Locked entry; **not load-bearing** — it wouldn't become a design invariant and doesn't constrain other nodes; **not preference-shaped** — no taste, appetite, or priority call that belongs to the user. An auto-close may never resolve against or reinterpret a Locked entry — anything that touches one runs the spec-amendment escalation, which is always the user's. A self-close takes the normal depth tag plus a marker — `[resolved: mechanism (auto)]` — and its own log row, `move: auto`. **Digest-and-veto:** with the next question you ask, list every auto-close since the last digest ("Self-closed: B2 → …, D1 → … — veto any"); a veto reopens the node as a `correct`. **The hard floor — always asked, in any mode:** the scope; anything that would become a design invariant (`DC-`); `decided`-given confirmations; every spec-amendment escalation; and the gate adjudications.
- `paranoid` — nothing closes without an explicit user confirm, even the obvious; state each close's depth tag as you close it; the Facilitator cadence trigger tightens to `≥ 3`; at the gate, re-confirm the `decided` givens and put each Tracer surfaced finding to the user individually rather than batch-triaging.
...
- **Prior-art check — a soft gate, not a hard one.** If `spec.md` exists but no `prior-art.md`, the buy-vs-build check hasn't run: remind the user and recommend `/prior-art-check` before design effort is sunk, but don't block — a user who has already decided to build proceeds. If `prior-art.md` exists, read its verdict; an `adopt` or `adopt-with-gaps` verdict means a matching solution was found and a full design grill may be the wrong next step — surface that prominently and confirm the user still wants to design before continuing.
- **Enrichment, when present:** the terrain pack the spec's frontmatter names (`terrain:`) — the durable map of the existing systems, conventions, and gates this design lands in; hold its path in the Position block, note staleness in one line when the repo has moved past its recorded SHA, and remember pack facts orient but never resolve. Also `spec/technical-parking-lot.md` (the deflected technical asides — the Planner's seed material) and the spec lane's state files (decision provenance). Parking-lot lines carrying a `[decided|leaning]` conviction tag — or, in a spec-only run, tagged §12 Carried-Forward lines — are copied into this lane's `givens.md` at seed time (origin `spec-lot`): the user's stated technical convictions, held as givens rather than open candidates. A run dir holding only `spec.md` — a conversation-born spec with no spec lane — is valid; enrichment is *when present*, not required.
...
- **Position block** — rewritten wholesale each turn: `posture` (the inherited tier), `granularity` (the decision-granularity mode — see above), `terrain` (the terrain pack path when the run has one, else `—`), `active`, `resume-target`, `turns-since-node-change`, `last-facilitator`.
...
4. **Update the map** — flip the active item's status if it resolved (**at most one standing item per user turn** — closing several on one answer is how a shallow answer slips through unexamined; `auto` closes are exempt from the cap, but each carries its own log row and must appear in a digest before the gate), and flip its given to `[consumed]` when the resolution folded one in; add any new concern; rewrite the Position block (bump or reset `turns-since-node-change`, update `active` / `resume-target`). The Locked block is untouched except on `amend`.
...
- **`steer`** — the user redirects the interview; honor it as sanctioned, log `move: steer`, `drift: none`. **Jump** (to a node or area): tangent bookkeeping — pause the current node, set `resume-target` if unset (nesting still fires its hard trigger), activate the target. **Skip**: mark the current node `[paused: user-skip]` and move to the next. **Defer** (a node or area): close it `[resolved: deferred]` on the user's call; an area-level defer flips each remaining item in one row, reviewed at the gate. **Wrap up**: drive to completion — present one triage digest of everything `unvisited`/`paused` with a recommendation each (defer or quick-close), let the user adjudicate, then run the normal pre-completion gate, never skipped. **Agenda**: print a compact map digest (areas with counts, active, resume-target, open flags); the map doesn't move. A granularity switch is also a `steer`: update the Position field and continue.
...
- **Cadence:** `≥ 4` turns since `last-facilitator` (count from the start if it never ran) — tighten to `≥ 3` if any turn since then was a `tangent`, `new-concern`, or `correct`, or whenever `granularity: paranoid`.
...
- how many closes were **`(auto)`** — each grounded, digested, and un-vetoed (an un-digested auto-close is a gate failure, not a summary line);
- any **terrain-pack corrections** the run surfaced — scout-reported places reality moved past the pack; suggest `/survey-terrain` to refresh;
...
- [resolved: mechanism (auto)] B3 <question> — <the artifact/procedure it resolved to; scout-verified>
- [paused: blocked-on-spec OQ-002] B4 <question> — reason added: <standing rationale>
- [dropped: superseded by B1] B5 <question> — reason added: <standing rationale>
- [unvisited] B6 <question to resolve> — given G1 (decided): <one line>
```
...
| 7 | E1 | auto | self-closed: metrics via existing statsd pipeline | repo-read; digest owed next question | none | D3 |
| 8 | D3 | steer | user: jump to K — rollout first | D3 paused; resume-target D3 | none | K1 |
