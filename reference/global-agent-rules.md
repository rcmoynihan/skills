...
- Do not create, open, link, or reference GitHub Issues during GitHub operations. This machine's
  workflows do not use issue tracking unless the current conversation explicitly says otherwise.
- No tracking issues, follow-up issues, or issue-backed TODOs. For follow-up work discovered during
  GitHub operations, use PR comments or review replies, or ask where the user wants it tracked.

## Accessing deployed infrastructure

When inspecting or querying deployed resources — Postgres, running pods, Kafka, AWS, Snowflake:

- **Staging and read-only by default.** Target staging and use inspection/read-only commands unless
  told otherwise.
- **Production is opt-in but then sticky.** If the user names production, or asks for live
  production debugging / questions / actions, that authorizes working against production for the
  rest of that line of work — don't re-ask before each command. Once they ask for an action, you
  likewise don't need to confirm every subsequent step. Still announce clearly destructive or
  irreversible operations (`DROP`/`DELETE`/`TRUNCATE`, deleting a topic, terminating an instance)
  before running them.
- **Discover the environment; don't assume.** There are several clusters, namespaces, AWS profiles,
  and Snowflake connections — find the right one (`kubectl config get-contexts`, `kubectl get ns`,
  `aws configure list-profiles`, `snow connection list`, plus the repo's deploy manifests) rather
  than guessing.
- **Kubernetes:** `kubectl --context <ctx> -n <ns> get pods | logs <pod> | describe pod <pod> | exec -it <pod> -- sh`.
- Reach in-cluster services (Postgres, Kafka, HTTP) by `kubectl port-forward` to localhost, then
  connect with `psql` / a Kafka client / `curl`. Use the AWS CLI for AWS, and `snow sql` for
  Snowflake. Run port-forwards in the background and kill them when done.
